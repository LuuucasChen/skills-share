...
从技能共享平台下载指定技能的 ZIP 压缩包，**直接保存到本地 platform_skills 目录，不进行解压**。适用于"仅下载"场景，下载后的文件保持 ZIP 格式。
...
1. 先调用 `query-skills` 获取技能列表
2. 从返回结果中提取目标技能的 `id` 字段
3. 调用下载接口获取 ZIP 二进制数据
4. 将 ZIP 保存到 `platform_skills/{技能名称}.zip`（不解压）
...
- **安装（install-skill）**：下载 ZIP 后自动解压，将内含文件部署到 skills 目录
